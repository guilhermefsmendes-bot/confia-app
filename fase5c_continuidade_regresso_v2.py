...
#
# IMPORTANTE:
# A condição daysSincePreviousOpen === 1 confirma apenas
# continuidade de utilização entre dois dias de calendário.
#
# Não atribuímos um episódio, emoção, necessidade ou
# comportamento específico ao dia anterior.
#
# NÃO cria:
# - storage
# - state
...
BACKUPS = {
    APP:
        Path("/tmp/App.tsx.before_fase5c_v2"),

    LOCALES["pt"]:
        Path("/tmp/pt.json.before_fase5c_v2"),

    LOCALES["en"]:
        Path("/tmp/en.json.before_fase5c_v2"),

    LOCALES["es"]:
        Path("/tmp/es.json.before_fase5c_v2"),

    LOCALES["fr"]:
        Path("/tmp/fr.json.before_fase5c_v2"),
}
...
def fail(message):
    print()
    print("=" * 78)
    print("ERRO — FASE 5C v2 NÃO APLICADA")
    print("=" * 78)
    print()
    print(message)
    print()
...
# ============================================================
# 3. GARANTIR QUE A TENTATIVA ANTERIOR NÃO ALTEROU A APP
# ============================================================

if "CONFIA 5C — CONTINUIDADE DO REGRESSO" in app_original:
    fail(
        "A Fase 5C já parece existir no App.tsx.\n"
        "Não vou duplicá-la."
    )

if "dailyMoment.continuityReturn." in app_original:
    fail(
        "Já existem referências dailyMoment.continuityReturn "
        "no App.tsx."
    )
...
# 4. LOCALIZAR A 5B
#
# A 5C entra imediatamente antes da 5B.
#
# Hierarquia:
#
# mensagem do Momento de Hoje
# ↓
# 5C continuidade do regresso
# ↓
# 5B semente para amanhã
# ↓
# 3D ação inteligente
# ============================================================
...
#
# - esta é a primeira abertura do dia;
# - a abertura anterior foi no dia de calendário anterior.
#
# Não conta sequências.
# Não atribui prémios.
# Não penaliza interrupções.
# ============================================================
...
if app_updated.count(
    ...
    fail(
        "O marcador da 5C não ficou único."
    )


# ============================================================
# 7. VALIDAR ORDEM VISUAL
# ============================================================
...
# 8. PERFORMANCE
#
# Nenhuma destas contagens pode mudar.
# ============================================================
...
# ============================================================
# 9. PROIBIR MECÂNICAS DE STREAK / RECOMPENSA
# ============================================================
...
# - "hier tu..."
#
# As mensagens referem apenas continuidade e memória
...
# ============================================================
# 11. VALIDAR TEXTO DAS TRADUÇÕES
#
# Aqui fazemos a verificação no conteúdo que o UTILIZADOR
# pode realmente ver, e não nos comentários do código.
# ============================================================
...
for language, block in translations.items():
    ...
    for phrase in forbidden_phrases[language]:

        if phrase in joined:
            fail(
                f"{language}: afirmação temporal "
                f"não suportada:\n{phrase}"
            )
...
for language, path in LOCALES.items():
    ...
    # A 5B tem de continuar presente.
    ...
    if set(block.keys()) != expected_keys:
        fail(
            f"{language}: chaves da 5C incorretas."
        )
...
# 13. BACKUPS
#
# Só chegamos aqui depois de TODAS as validações prévias.
# ============================================================
...
try:
    ...
    for token in tracked_tokens:
        ...
        if before != after:
            raise RuntimeError(
                f"Contagem alterada após escrita: {token}"
            )
    ...
    for language, path in LOCALES.items():
        ...
        if (
            ...
            or set(continuity.keys()) != expected_keys
        ):
            raise RuntimeError(
                f"5C inválida em {language}."
            )
...
print("=" * 78)
print(
    "CONFIA — FASE 5C v2 / CONTINUIDADE DO REGRESSO"
)
print("=" * 78)
print()

print("✓ Ciclo iniciado pela 5B fechado")
print("✓ Reconhece continuidade entre dias")
print("✓ Apenas em first_today")
print("✓ Exige daysSincePreviousOpen === 1")
...
print("✓ neutral")
print("✓ Não atribui registos específicos ao dia anterior")
print("✓ Nenhuma streak")
print("✓ Nenhuma contagem de sequência")
...
print("Backups:")
print(
    "  /tmp/App.tsx.before_fase5c_v2"
)
print(
    "  /tmp/pt.json.before_fase5c_v2"
)
print(
    "  /tmp/en.json.before_fase5c_v2"
)
print(
    "  /tmp/es.json.before_fase5c_v2"
)
print(
    "  /tmp/fr.json.before_fase5c_v2"
)
print()
