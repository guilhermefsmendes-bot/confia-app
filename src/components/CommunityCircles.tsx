...
const topics = ["ansiedade", "stress", "cansaco", "solidao", "sono", "familia", "relacoes"] as const;

export default function CommunityCircles({ posts, onOpenChat }: Props) {
  const { t, i18n } = useTranslation();
  const [active, setActive] = useState<string | null>(null);
  const grouped = useMemo(() => topics.map(id => ({
    id,
    label: t("communityTopics." + id),
    posts: posts.filter(post => post.topic === id).slice(0, 8),
  })).filter(group => group.posts.length > 0), [posts, t, i18n.language]);
  ...
  return (
    <section className="rounded-[28px] border border-[#E8DDD7] bg-white p-4 shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div><p className="text-[9px] font-black uppercase tracking-[.18em] text-[#B85F48]">{t("communityCircles.eyebrow")}</p><h3 className="text-sm font-black text-[#2F2926]">{t("communityCircles.title")}</h3><p className="mt-1 text-[10px] leading-4 text-slate-500">{t("communityCircles.subtitle")}</p></div>
      </div>
