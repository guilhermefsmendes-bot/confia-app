...
export default function PersonalMap({ onBack }: Props) {
  ...
  const events = useMemo(() => { recordPersonalAnalytics("personal_map_viewed"); return readPersonalEvents(); }, []);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FFF9F4] to-[#F1EAE4] p-5 pb-24">
      <button type="button" onClick={onBack} className="mb-5 flex items-center gap-2 text-[#795B50] font-semibold" aria-label={t("personalMap.backAria")}>
        <ArrowLeft size={17} /> {t("back")}
      </button>
      <div className="rounded-[30px] bg-white p-6 shadow-md border border-[#E8DDD4]">
        <div className="flex items-start gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#F4E7DF]"><Compass className="text-[#C97B5E]" /></div>
      ...
      <section className="mt-4 rounded-[26px] bg-white p-5 shadow-sm border border-[#E8DDD4]">
      ...
      <section className="mt-4 rounded-[26px] bg-white p-5 shadow-sm border border-[#E8DDD4]">
        <div className="flex items-center gap-2"><Sparkles size={17} className="text-[#C97B5E]" /><h2 className="font-black text-[#4A352F]">{t("personalMap.discoveries")}</h2></div>
        ...
        {events.length > 0 && <div className="mt-4 flex gap-2 text-xs text-[#806D65]"><Info size={14} /> {t("personalMap.explainable")}</div>}
      </section>
    </div>
  );
}
