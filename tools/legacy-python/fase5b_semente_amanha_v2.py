...
#
# Objetivo:
# criar expectativa suave para o dia seguinte dentro do
# Momento de Hoje.
#
# Usa apenas:
# - dailyContext.state
...
#
# BACKUPS:
#   /tmp/App.tsx.before_fase5b_semente_amanha_v2
#   /tmp/pt.json.before_fase5b_semente_amanha_v2
#   /tmp/en.json.before_fase5b_semente_amanha_v2
#   /tmp/es.json.before_fase5b_semente_amanha_v2
#   /tmp/fr.json.before_fase5b_semente_amanha_v2
# ============================================================
...
BACKUPS = {
    APP: Path("/tmp/App.tsx.before_fase5b_semente_amanha_v2"),
    LOCALES["pt"]: Path("/tmp/pt.json.before_fase5b_semente_amanha_v2"),
    LOCALES["en"]: Path("/tmp/en.json.before_fase5b_semente_amanha_v2"),
    LOCALES["es"]: Path("/tmp/es.json.before_fase5b_semente_amanha_v2"),
    LOCALES["fr"]: Path("/tmp/fr.json.before_fase5b_semente_amanha_v2"),
}
...
def fail(message):
    print()
    print("=" * 78)
    print("ERRO — FASE 5B v2 NÃO APLICADA")
    print("=" * 78)
    print()
    print(message)
    print()
...
if "CONFIA 5B — SEMENTE DE AMANHÃ" in app_original:
    fail("A Fase 5B já parece estar aplicada.")
...
if app_original.count(anchor) != 1:
    fail(
        "Não encontrei exatamente uma vez a âncora real "
        "da ação inteligente."
    )


# ============================================================
# 5. BLOCO SEMENTE DE AMANHÃ
# ============================================================
...
for marker in required_new:
    if marker not in app_updated:
        fail(
            f"Bloco 5B ficou incompleto:\n{marker}"
        )

if app_updated.count("CONFIA 5B — SEMENTE DE AMANHÃ") != 1:
    fail("Marcador 5B não ficou único.")

if app_updated.find("CONFIA 5B — SEMENTE DE AMANHÃ") > app_updated.find(
    "CONFIA 3D — AÇÃO INTELIGENTE DO DIA"
):
    fail(
        "A Semente de Amanhã não ficou antes da ação inteligente."
    )
...
for token in [
    ...
    if app_updated.count(token) != app_original.count(token):
        fail(
...
for language, path in LOCALES.items():
    ...
    daily_moment = data.get("dailyMoment")

    if not isinstance(daily_moment, dict):
        fail(
            f"{language}: bloco dailyMoment não existe."
        )
    ...
    daily_moment["tomorrow"] = translations[language]
...
for language in LOCALES:
    data = json.loads(locale_updated[language])
    ...
    if not isinstance(block, dict):
        fail(
            f"{language}: dailyMoment.tomorrow ausente."
        )
...
print()
print("=" * 78)
print("CONFIA — FASE 5B / SEMENTE DE AMANHÃ v2")
print("=" * 78)
print()

print("✓ Semente de Amanhã inserida")
print("✓ Imediatamente antes da ação inteligente")
...
print()
print("Backups:")
print("  /tmp/App.tsx.before_fase5b_semente_amanha_v2")
print("  /tmp/pt.json.before_fase5b_semente_amanha_v2")
print("  /tmp/en.json.before_fase5b_semente_amanha_v2")
print("  /tmp/es.json.before_fase5b_semente_amanha_v2")
print("  /tmp/fr.json.before_fase5b_semente_amanha_v2")
print()
print("PRÓXIMO PASSO:")
print("  npm run build")
print("=" * 78)
